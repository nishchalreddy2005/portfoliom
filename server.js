...
// --- GitHub Events Route (for Coding DNA visualization) ---
let cachedGithubEvents = null;
...
app.get('/api/github/events', async (req, res) => {
  try {
    ...
    if (cachedGithubEvents && (Date.now() - githubCacheTimestamp < GITHUB_CACHE_TTL)) {
      return res.json(cachedGithubEvents);
    }

    // Get github username from DB
    const content = await prisma.siteContent.findMany();
    const githubUrlEntry = content.find(c => c.key === 'hero_github_url');
    const githubUrl = githubUrlEntry ? githubUrlEntry.value : 'https://github.com/gvrnishchalreddy';
    const username = githubUrl.split('/').pop() || 'gvrnishchalreddy';

    // Fetch public events
    const eventsRes = await fetch(
      `https://api.github.com/users/${username}/events/public?per_page=100`,
      { headers: { 'User-Agent': 'portfolio-coding-dna' } }
    );

    if (!eventsRes.ok) {
      return res.status(502).json({ error: 'Failed to fetch GitHub events' });
    }
      ...
      if (event.type === 'PushEvent' && event.payload && event.payload.commits) {
        for (const commit of event.payload.commits) {
          commitNodes.push({
            ...
            language: null // will be enriched below
          });
        }
      }
    }

    // Fetch repo languages to color-code commits
    const repoNames = [...new Set(commitNodes.map(c => c.repo))];
    const repoLangs = {};
    
    await Promise.allSettled(
      repoNames.slice(0, 20).map(async (repo) => {
        try {
          ...
          if (langRes.ok) {
            ...
            const topLang = Object.keys(langs)[0] || null;
            repoLangs[repo] = topLang;
          }
        } catch {}
      })
    );

    // Enrich commits with language
    for (const node of commitNodes) {
      node.language = repoLangs[node.repo] || 'Unknown';
    }

    const result = { commits: commitNodes.slice(0, 80), username };
    cachedGithubEvents = result;
